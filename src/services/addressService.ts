...
export const addressService = {
    ...
    // Cập nhật địa chỉ
    async updateAddress(customerId: string, addressId: number, address: Address): Promise<Address> {
        const response = await fetch(`${API_BASE}/customer/${customerId}/${addressId}`, {
            method: 'PUT',
            headers: getAuthHeaders(),
            body: JSON.stringify(address)
        })
    ...
    async setDefaultAddress(customerId: string, addressId: number): Promise<Address> {
        const response = await fetch(`${API_BASE}/customer/${customerId}/${addressId}/set-default`, {
            method: 'PUT',
            headers: getAuthHeaders()
        })
        if (!response.ok) {
